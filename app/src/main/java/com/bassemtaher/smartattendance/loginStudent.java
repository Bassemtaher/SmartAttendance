...
public class loginStudent extends AppCompatActivity {
    ...
    private void validateName(final String fullname, final String number, final String pass)
    {
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            {
                if(!(dataSnapshot.child("student").child(fullname).exists()))
                {
                    HashMap<String,Object> userdataMap=new HashMap<>();
                    userdataMap.put("full name",fullname);
                    userdataMap.put("phone",number);
                    userdataMap.put("password",pass);
                    reference.child("student").child(fullname).updateChildren(userdataMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task)
                        {
                            if(task.isSuccessful())
                            {
                                Toast.makeText(loginStudent.this,"Congratulations your account has been created",Toast.LENGTH_SHORT).show();
                                loading.dismiss();
                                Intent intent=new Intent(loginStudent.this,scanActivity.class);
                            ...
                            {
                                loading.dismiss();
                                Toast.makeText(loginStudent.this,"network error please try again later",Toast.LENGTH_SHORT).show();
                ...
                {
                    Toast.makeText(loginStudent.this,"this"+fullname+"already exists",Toast.LENGTH_SHORT).show();
                    loading.dismiss();
                    Toast.makeText(loginStudent.this,"please try again using your wright name",Toast.LENGTH_SHORT).show();
